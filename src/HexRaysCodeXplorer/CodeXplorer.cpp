...
static ssize_t idaapi gr_callback(void *ud, const int code, va_list va)
{
    ...
    switch (code)
    {
            ...
            // out: success
        {
            ...
            for (auto it = fg->begin_edges(); it != end; ++it)
            {
...
static bool idaapi display_ctree_graph(void *ud)
{
    ...
    if (widget)
    {
        msg("Ctree Graph window already open. Switching to it.\n");
...
plugmod_t *idaapi init(void)
{
    ...
    {
        msg("[nHexRaysCodeXplorer] Hex-Rays Decompiler not found!\n");
    ...
    if (-1 == g_replace_id)
    {
        g_replace_id = register_place_class(&replace_template, PCF_EA_CAPABLE, &PLUGIN);
    }
...
void idaapi term(void)
{
    if (inited)
    {
        ...
        for (auto &k_action_desc : k_action_descs)
        {
            unregister_action(k_action_desc.name);
        }
    }
...
plugin_t PLUGIN =
{
    ...
    "HexRaysCodeXplorer by @REhints",   // multiline help about the plugin
    "CodeExplorer",             // the preferred short name of the plugin (PLUGIN.wanted_name)
